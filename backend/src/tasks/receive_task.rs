        ...
        match serde_json::from_str::<MessageDTO>(msg.as_str()) {
            Ok(msg_dto) => match msg_dto {
                MessageDTO::AddPlayer { player } => {
                    ...
                    loop {
                        // println!("-.-49");
                        if state.rooms.lock().await.get(&room_send).unwrap().tx.send(SenderMessage::LoggedIn).is_ok() {
                ...
                MessageDTO::StartGame { room_id } => if let Some(room) = state.rooms.lock().await.get(&room_id) {
                    ...
                    *room.status.lock().await = state_vec.to_owned();
                    ...
                    loop {
                        if room.tx.send(SenderMessage::StartGame).is_ok() {break;}
                    }
                },
